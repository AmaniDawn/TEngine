---
name: fantasy-net
description: Fantasy 服务器/Unity 客户端框架（Fantasy.Net / Fantasy.Unity，C#）开发与代码审查指导。触发场景：(1) 编写或审查 Fantasy 服务器代码或 Unity 客户端 Fantasy 代码，(2) ECS 实体/组件/系统、Scene/SubScene、FTask 异步，(3) 网络 Handler/消息/协议（Outer/Inner/Address/Roaming），(4) Address 或 Roaming 路由、跨服事件 SphereEvent，(5) Control Center 服务发现、Namespace/WorldGroup/World 隔离、动态 Scene 注册/路由，(6) Fantasy.config、Scene/数据库（MongoDB）访问、HTTP Controller/Service，(7) Session/客户端连接逻辑、分布式运行时架构。即使用户未明确提到 Fantasy，只要是 Fantasy 相关的开发、排错、合规检查、风险分析、最佳实践，也应使用此技能。触发词：Fantasy, Fantasy.Net, Fantasy.Unity, FTask, ECS, Scene, SubScene, MessageHandler, Session, Roaming, Address, SphereEvent, ServiceDiscovery, WorldGroup, Fantasy.config, GameClient
---
...
本 skill 的 references/templates 以 Fantasy 原生项目结构描述，在 TEngine 中的对应位置如下：

- **Fantasy.Unity 客户端框架**：`UnityProject/Assets/GameScripts/HotFix/Fantasy.Unity/`（已拆分进热更程序集，路径前缀与原生 `Fantasy/` 对应）
- **Fantasy 服务器**：`GameServer/Server/`（`Entity/` 实体层、`Main/` 启动入口）
- **TEngine 客户端网络入口**：`UnityProject/Assets/GameScripts/HotFix/GameLogic/DataCenter/GameClient.cs`（`GameClient` 单例，封装 `Scene`、`Session`、收发与重连）
- **Fantasy 框架更新后的手动修改清单**：`UnityProject/Assets/GameScripts/HotFix/GameLogic/DataCenter/ReadMe.md`（更新 Fantasy 后必读，含 `Scene.MessageDispatcherComponent` 改 public、`MessageDispatcherComponent` 客户端回调、`Log` 条件宏、`ProtoBufHelper` 日志回调等需手动恢复的改动）

> 注：references/templates 为 Fantasy 通用知识，保留英文原样；遇到路径相关描述时，以 TEngine 实际结构为准（见上方对应关系）。
