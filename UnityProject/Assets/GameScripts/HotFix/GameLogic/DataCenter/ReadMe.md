...
> ps: Fantasy.Unity 建议是不进热更程序集，因为这个基本是稳定代码，没有特别严重Bug应该不会有什么改动，且它热更是必须带着Assembly-CSharp一起热更的。不过当前是已经把程序集拆分好，放进了热更程序集里，如无特殊需求，可以自己把Fantasy.Unity和Assembly-CSharp取消热更。
>
...
**检查内容**: 确认 `Debug`、`Info`、`Warning`、`Error` 方法的 `[Conditional]` 属性与 `TEngine.Log` 保持一致（`Trace` 和 `TraceInfo` 除外）
